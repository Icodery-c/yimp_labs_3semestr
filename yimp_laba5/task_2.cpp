...
void addEdge(std::vector<Edge>& edges, int u, int v, double weight) {
    for (auto& e : edges) {
...
void saveAsMatrix(const std::filesystem::path& filepath, const std::vector<Edge>& edges, int n) {
    ...
    file.close();
    std::cout << "Матрица весов сохранена: " << std::filesystem::absolute(filepath) << std::endl;
}
...
void saveGraph(const std::filesystem::path& filepath, const std::vector<Edge>& edges, int n) {
    ...
    for (int i = 0; i < n; ++i) {
        file << i;
        for (const auto& neighbor : adj[i]) {
            file << " " << neighbor.first << " " << std::fixed << std::setprecision(2) << neighbor.second;
        }
        file << "\n";
    }

    file.close();
    std::cout << "Взвешенный граф сохранён в файл: " << std::filesystem::absolute(filepath) << std::endl;
}
...
int main() {
    ...
    while (true) {
        ...
        std::cout << "5. Сохранить как матрицу весов" << std::endl;
