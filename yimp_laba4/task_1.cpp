...
TreeNode* insertIterative(TreeNode*& root, TreeNode* z) {
    TreeNode* x = root;
    ...
    while (x != nullptr) {
        ...
        if (z->value < x->value)
            ...
            x = x->right;
				else {delete z; return root; }
    }

    z->parent = parent;

    if (parent == nullptr) { root = z; }

		else if (z->value < parent->value) { parent->left = z; }

		else if (z->value > parent->value) {parent->right = z; }
...
int main () {
	...
	for (int i = 0; i < N; i++) {
		...
		value = getValInput();
		root = insertIterative(root, CreateNode(value));
